...
<form action="{{ route('convenios.update', ['pk_id_conv' => $convenio->pk_id_conv]) }}" method="post">
    ...
    <div class="form-group">
        ...
        <input type="text" id="nome_conv" name="nome_conv" class="form-control" value="{{ $convenio->nome_conv }}">
    </div>

    <div class="form-group">
        <label for="ans_conv">Registro ANS</label>
        <input type="text" id="ans_conv" name="ans_conv" class="form-control" value="{{ $convenio->ans_conv }}">
    </div>
